...
export default function PortfolioModal({ isOpen, onClose, project }: PortfolioModalProps) {
  ...
  const getScreenshotUrl = () => {
    if (project.title === "ePosterLive") return "/images/eposterslive-screenshot.jpeg"
    if (project.title === "InstaJudge") return "/images/instajudge-screenshot.jpeg"
    return null
  }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-primary">{project.category}</DialogDescription>
        </DialogHeader>
